...
import mtl_dnn_sym as models
from stlpoortask_mtl_dnn_sym_configure import FLAGS
...
def select_tasks_load_inds(features_normalized, labels):
    ...
    train_inds, test_inds, valid_inds, lr_test_auc = get_train_test_valid_inds()
    for kk in range(K):
        id = tasks_ids[kk]
    ...
    return tuple([train_x_t, train_y_t, valid_x_t, valid_y_t, test_x_t, test_y_t]), lr_test_auc
...
def record_configure_files(argv):
    ...
    copyfile("stlpoortask_mtl_dnn_sym_configure.py", FLAGS.train_dir + "stlpoortask_mtl_dnn_sym_configure.py")
    copyfile("stlpoortask_mtl_dnn_sym_run.py", FLAGS.train_dir + "stlpoortask_mtl_dnn_sym_run.py")
    copyfile("mtl_dnn_sym.py", FLAGS.train_dir + "mtl_dnn_sym.py")
...
def get_train_test_valid_inds():
    ...
    for kk in range(K):
        ...
        lr_test_aucs[kk] = mat_contents['results'][index_task[kk]][0][0][9]
        pos_all[kk] = mat_contents['results'][index_task[kk]][0][0][-3][0]

    return train_inds, test_inds, valid_inds, lr_test_aucs

def run_training(dataset_tuple, task_id):
    ...
    with tf.Graph().as_default():
        ...
        logits, regularizers_t, linear_pred_t, var_list1, var_list_t, regularizers_l1 \
        ...
        train_op = models.training(obj_value, FLAGS.learning_rate1, FLAGS.learning_rate_t,
                                   var_list1, var_list_t)
        ...
        save_dict = {}
        for ll in FLAGS.Layer_last_restore:
            save_dict["weights" + str(ll + 1)] = var_list1[2*ll]
            save_dict["biases" + str(ll + 1)] = var_list1[2*ll+1]
        ...
        for step in xrange(FLAGS.max_steps):
            ...
            if  (step + 1) == FLAGS.max_steps:
                checkpoint_file = os.path.join(FLAGS.train_dir, 'checkpoint' + str(task_id))
...
def main(argv):
    ...
    # transformer = random_projection.SparseRandomProjection(n_components=FLAGS.FEATURE_SIZE)
    # X_new = transformer.fit_transform(features)
    # print X_new.shape
    ...
    for ii in range(1000):
        ...
        dataset_tuple, lr_test_auc = select_tasks_load_inds(features_normalized, labels)
        ...
        lr_maxtestauc = lr_test_auc[0][0][0]
        if nn_test_auc > lr_maxtestauc:
            print ("\nNonlinear exist,  AUC of task %4d, DNN %f, LR %f\n") %(ii, nn_test_auc, lr_maxtestauc)
            task_ids_store.append(ii)
            string_flag = "Nonlinear exist "
        else:
            print ("\nLR better: AUC of task %4d, DNN %f, LR %f\n") % (ii, nn_test_auc, lr_maxtestauc)
            string_flag = "LR better "

        results = string_flag + "AUC of task {0:d}, DNN {1:f}, LR {2:f} \n".format(ii, nn_test_auc, lr_maxtestauc)
        f1.write(results)
        task_run_auc_nn.append(nn_test_auc)
        task_run_auc_lr.append(lr_maxtestauc)
    print "NN better than LR tasks ", task_ids_store
    results_final_saved = tuple([task_ids_store, tasks_run, tasks_skiped, task_run_auc_nn, task_run_auc_lr])
...
#CUDA_VISIBLE_DEVICES= nohup python -u stlpoortask_mtl_dnn_sym_run.py > nohup.STNN_fold2_2 2>&1 &
if __name__ == "__main__":
    '''multi task - multi layer fully connected neural network
       ...
       restore from autoencoder for multiple layers
       run all tasks one by one, using mtl_dnn_sym methods
       '''
