...
def inference(placeholder_tuple, hidden_units, FEATURE_SIZE):
    ...
    for ll in range(L):
        ...
        if ll == 0:
            ...
            with tf.name_scope('hidden' + str(ll + 1)):
                ...
                hidden_l = [None] * K  # output value of layer 1 for K tasks
                for kk in range(K):
    ...
    for kk in range(K):
        with tf.name_scope('softmax_linear_task' + str(kk)):
            weights_t[kk] = tf.Variable(
                tf.truncated_normal([hidden_units[L-1], NUM_CLASSES],
            ...
            # add dropout
            hidden_drop_t[kk] = tf.nn.dropout(hidden_last[kk], dropout_tasks[kk])
    ...
    var_list_t = [None] * K
    L_hidden_vars = [weights[ll] for ll in range(1, L)] + [biases[ll] for ll in range(1, L)]

    for kk in range(K):
        var_list_t[kk] = L_hidden_vars + [weights_t[kk], biases_t[kk]]

    return logits, regularizers_t, linear_pred_t, var_list1, var_list_t, regularizers_l1
...
def training(obj_value, learning_rate1, learning_rate_t, var_list1, var_list_t):
    ...
    train_op1 = optimizer1.minimize(obj_value, global_step=global_step, var_list=var_list1)
    ...
    for kk in range(K):
        train_op_tasks[kk] = optimizer_task[kk].minimize(obj_value, global_step=global_step, var_list=var_list_t[kk])

    # train_op_tasks = optimizer_task1.minimize(obj_value, global_step=global_step, var_list=var_list_task1)
    # train_op_task2 = optimizer_task2.minimize(obj_value, global_step=global_step, var_list=var_list_task2)
    train_op = train_op1
    for kk in range(K):
...
def evaluation(linear_pred, y_true):
    ...
    return eval_correct_t, label_pred_t, label_true_t, tuple([tp, tn, fp, fn])         # TODO debug
